...
if frame > 1
       ...
       % each pixel of response_pwp loosely represents the likelihood that the target (of size norm_target_sz) is centred on it
       response_pwp = getCenterLikelihood(likelihood_map, p.norm_target_sz);

       response = (1 - p.merge_factor)*response_cf + p.merge_factor*response_pwp;
    ...
    %%  "Unreliability" and "Reliability" check
      ...
      if  ( colorScore < color_low_thres * colorAver )||( hogScore < hog_low_thres * hogAver )    
          ...
          for i = 1 : opt.highest_num
             candidate_pwp = getSubwindow( im, particle_pos(i,:), p.norm_pwp_search_area, pwp_search_area);
             candidate_cf = getSubwindow( im, particle_pos(i,:), p.norm_bg_area, bg_area);
             [responseFinal{i}, responseCF{i}, score(i)] = confidenceCalculate(candidate_pwp, candidate_cf, bg_hist, fg_hist, p, hann_window, hf_num, hf_den);
          ...
          [~, ID] = max(score);                                              
          % Calculate candidates' hogScore and colorScore
